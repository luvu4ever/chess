...
void login_account(int sock) { 
    ...
    // Use the existing function to send the login request to the server
...
void register_account(int sock) { 
    ...
    // Use the existing function to send the registration request to the server
    ...
    if (strcmp(buffer, "REGISTER_SUCCESS") == 0) {
        ...
        getch();
        clear();
        // Call function to display the login menu here (you will need to implement display_login_menu)
        display_login_menu(sock);
    } else {
